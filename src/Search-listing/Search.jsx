...
function list() {
  ...
  const [totalProjects, setTotalProjects] = useState(16);
  ...
  const handleSort = (key) => {
    ...
    const sorted = [...searchResults].sort((a, b) => {
      if (key === 'name') {
        ...
        return parseFloat(a.tag_price || 0) - parseFloat(b.tag_price || 0);
      }
      return 0;
    });
  ...
  <Container>
    <Row className="blog-select mb-5">
      ...
      <Col md={6} className="search-filterr">
        <DropdownButton
          id="Sort by"
          title={sortKey ? `Sort by: ${sortKey === 'name' ? 'Name' : 'Price'}` : 'Sort by'}
          variant="outline-light"
